...
export class TSRHandler {
	...
	private async _updateDevices(): Promise<void> {
		...
		if (peripheralDevice) {
			...
			const activeRundownPlaylist = rundownPlaylists.findOne((i: any) => {
				return i.studioId === peripheralDevice.studioId && i.activationId
			})
						...
						if (anyChanged) {
							...
							ps.push(
								keepTrack(this._removeDevice(deviceId), 'remove_' + deviceId).then(() => {
									return keepTrack(
										this._addDevice(deviceId, deviceOptions, activeRundownPlaylistId),
										're-add_' + deviceId
									)
								})
							)
	...
	private async _addDevice(
		deviceId: string,
		options: DeviceOptionsAny,
		activeRundownPlaylistId: string | undefined
	): Promise<any> {
